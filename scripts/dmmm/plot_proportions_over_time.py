...
health_metrics = {
  'answers-per-question': lambda df: df['num_answers'] / df['num_questions'],
  'percent-accepted-ans': lambda df: df['num_with_acc_ans'] / df['num_questions'],
  'percent-answered':     lambda df: (df['num_questions'] - df['num_unanswered']) / df['num_questions'],
...
df = frame
fig, ax = plt.subplots(1, 1, figsize=(4.32, 3.24))
labels = []
markers = itertools.cycle(('o', '^', 's', '*', 'd'))
for label, df in df.groupby('topic'):
    df.plot(x='month', y='probability', kind='line', ax=ax, markevery=5,
        marker=next(markers))
